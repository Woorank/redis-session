...
module.exports = {
  ...
  middleware: function (redis, options) {
    ...
    return function (req, res, next) {
      ...
      (
        typeof redis.getAsync === 'function'
          ? redis.getAsync(sessionNamespace + sessionSeparator + sessionId)
          : new Promise(
              (resolve, reject) => {
                redis.get(
                  sessionNamespace + sessionSeparator + sessionId,
                  (e, serializedSession) => {
                    if (e) {
                      return reject(e);
                    }

                    return resolve(serializedSession);
                  }
                );
              }
            )
